...
class _FirstQuestionScreenState extends State<FirstQuestionScreen> {
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () {
        if (selfDiscoveryCon.index == 1) {
          Get.back();
        }
        return Future.value(false);
      },
      child: GetBuilder<SelfDiscoveryCon>(initState: (state) {
        selfDiscoveryCon.selectedAns = 0;
        selfDiscoveryCon.selectedQueAnsList.clear();
        selfDiscoveryCon.getQuestionList();
      }, builder: (ctrl) {
        return Stack(
          children: [
            Scaffold(
            ...
            Obx(() =>
                ctrl.isLoading.value ? const AppProgress() : const SizedBox())
          ],
        );
      }),
    );
